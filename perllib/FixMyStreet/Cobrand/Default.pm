...
sub find_closest {
    ...
    if ($j && $j->{resourceSets} && $j->{resourceSets}[0]{resources}[0]{name}) { # Bing results
        return FixMyStreet::Geocode::Address->new($j->{resourceSets}[0]{resources}[0]);
    } elsif ($j) {
        return FixMyStreet::Geocode::Address::OSM->new($j);
    }
...
sub find_closest_address_for_rss {
    ...
    if ($j && $j->{resourceSets} && $j->{resourceSets}[0]{resources}[0]{name}) { # Bing results
    ...
    } elsif ($j && $j->{display_name}) { # OSM results
        ...
        $str .= sprintf(_("Nearest road to the pin placed on the map (automatically generated by OpenStreetMap): %s"),
            join( ', ', @address ) ) if @address;
    }
