...
sub send_email {
    ...
    try {
        ...
        # Format and save the email to the log file
        log_formatted_email($email);
    } catch {
...
sub log_formatted_email {
    my ($email) = @_;

    # Format the email message
    my $message = $email->as_string;
    my $formatted_message = format_email_message($message);

    # Save the formatted message to the log file
    save_email_to_log($formatted_message);
}

sub format_email_message {
    my ($message) = @_;

    # Split the message into lines
    my @lines = split("\n", $message);

    # Filter lines up to the specified line
    my @filtered_lines = ();
    foreach my $line (@lines) {
        last if $line =~ /do not reply to it/i;  # Stop filtering when the specified line is found
        push @filtered_lines, $line;
    }

    # Format each line
    my @formatted_lines = map { "> $_" } @filtered_lines;

    # Join the formatted lines back into a string
    my $formatted_message = join("\n", @formatted_lines);

    return $formatted_message;
}

sub save_email_to_log {
    my ($formatted_message) = @_;

    # Open the log file in append mode
    open(my $log_file, '>>', 'logmail.txt') or die "Cannot open logmail.txt: $!";

    # Write the formatted message to the log file
    print $log_file $formatted_message;
    print $log_file "\n\n";  # Add empty lines between emails
